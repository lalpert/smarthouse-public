...
class Thermostat(object):
    ...
    def therm_info(self):
        if self.inside_cache is None or time.time() - self.last_inside_fetch > self.INSIDE_REFRESH_SECS:
    ...
    def outside_temp(self):
        return -1
        #if self.outside_cache is None or time.time() - self.last_outside_fetch > self.OUTSIDE_REFRESH_SECS:
    ...
    def set_temp(self, temp):
        requests.post(self.imp_url + "/set", json={"password": self.password, "temp": str(temp)}, timeout=3)
